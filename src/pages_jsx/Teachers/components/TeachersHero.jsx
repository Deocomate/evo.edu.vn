// src/pages_jsx/Teachers/components/TeachersHero.jsx
import React from 'react';
import Image from 'next/image';

function TeachersHero() {
    return (
        <section className="relative w-full h-[70vh] sm:h-screen flex items-center justify-center text-white">
            <Image
                src="/assets/images/evo-images/4.jpg"
                alt="Đội ngũ giảng viên tại Evo Education"
                fill
                className="object-cover"
                quality={90}
                priority
            />
            <div className="absolute inset-0 bg-black/50 z-10" />
            <div className="relative z-20 text-center px-4">
                <h1 className="text-4xl md:text-5xl lg:text-5xl font-extrabold tracking-tight drop-shadow-md">
                    Đội Ngũ Giảng Viên Chuyên Môn
                </h1>
                <p className="mt-4 text-lg md:text-xl lg:text-xl max-w-3xl mx-auto drop-shadow-md">
                    Nền tảng cho sự thành công của mỗi học viên bắt nguồn từ những người thầy cô tận tâm, giàu kinh nghiệm thực chiến và khả năng truyền cảm hứng sáng tạo.
                </p>
            </div>
        </section>
    );
}

export default TeachersHero;